...
server_t *server_find(char *name)
{
  server_t *s;
  node_t *n;
  uint32_t hash = shash(name, sizeof(name));
...
user_t *user_find(char *nick)
{
  user_t *u;
  node_t *n;
  uint32_t hash = shash(nick, sizeof(nick));
...
myuser_t *myuser_add(char *name, char *pass, char *email)
{
  ...
  mu->hash = shash(name, sizeof(name));
...
mychan_t *mychan_add(char *name, char *pass)
{
  ...
  mc->hash = shash(name, sizeof(name));
